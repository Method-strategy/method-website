...
## July 2026 — GA4 anomaly triage: 404 URL identification + old WP title investigation
- Anomaly 1 ("Page not found — Method": 7 views / 4 users, week 1). Root cause: real humans hitting legacy WP paths not covered by initial `_redirects` audit. Probe against production identified 6 missing legacy-path groups. Data (which URLs) was already in GA4 as `page_path` on the existing `page_view`; secondary-dimension pivot answers without any code change.
- Anomaly 2 ("Method Marketing Agency | Atlanta, GA": 3 views). Root cause: same GA4 measurement ID (G-7F2PPZPXSK) was on the old WP site pre-migration → bfcache restore / stale Chrome preload of the pre-launch cached HTML fires `page_view` against our property with the old title. Confirmed no legacy origin still serves it (DNS-probed every likely subdomain — none resolve; methoddigitalmarketing.com Cloudflare 301s to new domain). Nothing in current codebase contains that string. Transient by nature; will decay to zero as caches expire.
- Fixes shipped (both requested): (a) Six second-wave legacy 301s in `frontend/public/_redirects` — `/services`, `/contact`, `/home`, `/feed`, `/category/*`, `/sitemap_index.xml` (last is a 200 rewrite so Yoast-era crawlers parse the new sitemap in-place); (b) Dedicated GA4 `not_found` event fires from `NotFound.jsx` alongside `page_view`, carrying `page_path`, `page_location`, `page_title`, `page_referrer`. Hostname-guarded like all other analytics.
